...
def apply_column_exclusion(df: pd.DataFrame, config: Dict, table: str):
    if config["primary_key"] in config["exclude_columns"]:
        raise TulonaFundamentalError("Cannot exclude primary key/join key from data comparison")
    ...
    if len(missing_cols) > 0:
        raise ValueError(f"Columns [{missing_cols}] to be excluded are not present in {table}")
